mod private
{
  ...
  /// Dash line uses the following pattern:
  /// Dash1, Gap1, Dash2, Gap2
  /// 
  /// Different variants of the enum are used to control this pattern
  #[ derive( Debug, Clone, Copy ) ]
  pub enum DashPattern
  {
    /// All dashes and gaps are have the same width
    V1( f32 ),
    /// Specifies the width for Dash1 and Gap1 respectively
    /// Dash1 == Dash2
    /// Gap1 == Gap2
    V2( [ f32; 2 ] ),
    /// Specifies the width for Dash1, Gap1, Dash2 respectively
    /// Due to odd number of values, at each odd itereation, the pattern flip dashes and gaps 
    /// resultin in:
    /// dash gap dash (flip) gap dash gap
    V3( [ f32; 3 ] ),
    /// Specifies the width for Dash1, Gap1, Dash2 and Gap2 respectively
    V4( [ f32; 4 ] )
  }
