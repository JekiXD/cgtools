...
) -> vec3f
{
...

  {
    var tleft = 100.0;
    var tright = 0.0;
    var pleft = p_in + rd_in * tleft;
    var pright = p_in;

    var hleft = 1.5 - sea_noise( pleft );
    var hright = 1.5 - sea_noise( pright );
    var dleft = abs( pleft.y - PLANE_P.y );
    var dright =  abs( pright.y - PLANE_P.y );
    ...
    if dleft - hleft > 0.0
    {
      for( var i = 0; i <  32; i++)
      {
        ...
        let delta_l = hleft - dleft;
        let delta_r = hright - dright;

        let tmid = mix( tleft, tright, delta_l / ( delta_l - delta_r ) );
        let pmid = p_in + tmid * rd_in;
        let dmid = abs( pmid.y -  PLANE_P.y );
        let hmid = 1.5 - sea_noise( pmid );

        if abs( dmid - hmid ) < 0.001
        {
          pleft = pmid;
          break;
        }
